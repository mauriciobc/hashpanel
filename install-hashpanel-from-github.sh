...
# Script seguro para instalar HashPanel via GitHub
# Uso: ./install-hashpanel-from-github.sh [--yes] [--production]
#      ./install-hashpanel-from-github.sh [--inspect] [--production]
...
if ! echo "$GITHUB_REPO" | grep -qE '^[a-zA-Z0-9_-]+/[a-zA-Z0-9._-]+
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
CYAN='\033[0;36m'
...
TEMP_SCRIPT=$(mktemp /tmp/hashpanel-install-XXXXXX.sh)
...
echo ""
info "Baixando script de instalação do HashPanel..."
...
if [ "$HAS_SHA256SUM" = true ]; then
    ...
    if [ -n "$EXPECTED_SHA256" ]; then
        ...
        warning "Para segurança adicional, defina EXPECTED_SHA256 antes de executar"
    fi
else
...
echo ""
info "Executando script de instalação..."
echo ""
...
; then
    echo "❌ Erro: GITHUB_REPO inválido. Use o formato: owner/repo"
    exit 1
fi

# Validate GITHUB_BRANCH format (basic alphanumeric with -, _, /)
if ! echo "$GITHUB_BRANCH" | grep -qE '^[a-zA-Z0-9/_.-]+
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
CYAN='\033[0;36m'
...
; then
    echo "❌ Erro: GITHUB_BRANCH inválido"
    exit 1
fi
